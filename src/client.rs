...
use tonic::{transport::Uri, codegen::http::uri::Authority};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let authority = ("[::1]:".to_string() + port_str).parse::<Authority>().unwrap();
    ...
    let uri = Uri::builder()
        ...
        .path_and_query("")
        .build();
    let channel = tonic::transport::Channel::builder(uri.unwrap()).connect().await?; 
